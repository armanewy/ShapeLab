...
use shape_core::{Aabb, Transform3};
...
fn candidate(id: &str, silhouette: [f32; 3]) -> AssetCandidateInput {
...
fn candidate_set() -> Vec<AssetCandidateInput> {
    vec![
        candidate("asset-a", [0.12, 0.18, 0.24]),
        candidate("asset-b", [0.24, 0.32, 0.16]),
        candidate("asset-c", [0.36, 0.22, 0.48]),
        candidate("asset-d", [0.48, 0.56, 0.28]),
        candidate("asset-e", [0.62, 0.36, 0.64]),
        candidate("asset-f", [0.74, 0.68, 0.40]),
        candidate("asset-g", [0.86, 0.46, 0.76]),
        candidate("asset-h", [0.52, 0.78, 0.88]),
    ]
}

#[test]
fn duplicates_collapse_before_selection() {
    let mut simple = candidate("simple", [0.42, 0.42, 0.42]);
...
fn invalid_candidate_is_hard_rejected() {
    let mut invalid = candidate("invalid", [0.3, 0.3, 0.3]);
...
fn excessive_detail_is_penalized_without_hard_rejection() {
    let simple = candidate("simple", [0.4, 0.4, 0.4]);
    let mut detailed = candidate("detailed", [0.4, 0.4, 0.4]);
...
fn same_recipe_under_different_tessellation_does_not_dominate_scoring() {
    ...
    for index in 0..7 {
        let mut variant = candidate(&format!("same-{index}"), [0.44, 0.44, 0.44]);
