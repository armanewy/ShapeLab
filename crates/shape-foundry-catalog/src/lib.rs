...
mod tests {
    ...
    fn advisory_and_runtime_controls_do_not_change_geometry_identity_but_required_controls_do() {
        ...
        required_document
            .control_state
            .insert("body_width".to_owned(), ControlValue::Scalar(1.6));
    ...
    fn effective_family_request_contains_defaulted_slots() {
        ...
        document.control_state.remove("body_width");

        let output = shape_foundry::compile_foundry_document(&document, &fixture)
            .expect("defaulted required control should compile");
        assert_eq!(
            output.family_request.parameters.get("body_width"),
            Some(&shape_family_compile::FamilyValue::Scalar(1.2))
        );
    }
    ...
    fn local_override_divergence_is_reported() {
        ...
        assert_eq!(
            output.control_divergence.get("body_width"),
