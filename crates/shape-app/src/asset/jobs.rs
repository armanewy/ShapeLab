...
fn generate_candidates(
    ...
    let mut candidate_by_id = BTreeMap::<String, SearchAssetCandidate>::new();
    let mut score_inputs = Vec::new();
    for candidate in output.candidates {
        if program_changes_locked_scope(recipe, &candidate.program) {
            continue;
        }
        let id = candidate.id.to_string();
        match compile_asset(&candidate.recipe) {
            Ok(artifact) => {
                score_inputs.push(scoring_input(
                    &id,
                    &candidate.recipe,
                    &artifact,
    ...
    for (slot, scored) in report
        ...
        .enumerate()
    {
        let Some(candidate) = candidate_by_id.remove(&scored.id) else {
            continue;
        };
        selected.push(app_candidate_from_search(recipe, candidate, scored, slot));
    }
...
fn scoring_input(
    ...
    input.detail_count = artifact
        ...
        .count();
    input.symmetry_score = symmetry_score(recipe, artifact);
