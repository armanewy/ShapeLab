...
use crate::asset::{
    AssetAppCommand, AssetParameter, AssetParameterGroup, AssetPart, AssetUiState, ParameterId,
...
pub(crate) fn parameter_command(
    ...
    parameter_value_changed(parameter, clamped)
        .then_some(AssetAppCommand::SetParameter(parameter.id, clamped))
}
...
pub(crate) fn parameter_lock_command(
    ...
    (parameter.locked != locked).then_some(AssetAppCommand::SetParameterLock(parameter.id, locked))
}
...
pub(crate) fn part_presence_command(
    ...
    (part.optional && !locked && part.enabled != proposed_enabled).then_some(
        AssetAppCommand::ToggleOptionalPart(part.id, proposed_enabled),
    )
}
...
pub(crate) fn part_lock_command(
    ...
    (was_locked != locked).then_some(AssetAppCommand::SetPartLock(part, locked))
}
...
pub(crate) fn subtree_lock_command(
    ...
    (was_locked != locked).then_some(AssetAppCommand::SetSubtreeLock(part, locked))
}
...
pub(crate) fn topology_lock_command(
    ...
    (was_locked != locked).then_some(AssetAppCommand::SetTopologyLock(definition, locked))
}
