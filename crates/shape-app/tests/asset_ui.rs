...
fn asset_panel_helpers_emit_commands() {
    assert_eq!(
        ...
        Some(AssetAppCommand::SelectPart(Some(PartInstanceId(2))))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(AssetAppCommand::SwitchBranch(AssetRevisionId(2)))
    );
    assert_eq!(
...
fn candidate_summary_and_progress_are_stable() {
    ...
    state.active_job = Some(AssetJobProgress {
        job_id: AssetJobId(8),
        kind: AssetUiJobKind::CandidateSearch,
...
fn lock_behavior_blocks_edits_and_deduplicates_lock_commands() {
    ...
    assert_eq!(
        ...
        Some(AssetAppCommand::SetParameter {
    ...
    assert_eq!(
        ...
        Some(AssetAppCommand::SetLock {
            target: AssetLockTarget::Parameter(ParameterId(1)),
            locked: true,
        })
    );
    ...
    assert_eq!(
        ...
        Some(AssetAppCommand::SetLock {
            target: AssetLockTarget::Instance(PartInstanceId(4)),
            locked: true,
        })
    );
    assert_eq!(
        ...
        Some(AssetAppCommand::SetLock {
            target: AssetLockTarget::Subtree(PartInstanceId(4)),
            locked: true,
        })
    );
    assert_eq!(
        ...
        Some(AssetAppCommand::SetLock {
            target: AssetLockTarget::Topology(PartDefinitionId(7)),
            locked: true,
        })
    );
}
