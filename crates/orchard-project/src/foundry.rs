...
use orchard_asset::{AssetRecipe, RevisionId};
use orchard_foundry::{
    ...
    GeneratedRecipeSnapshot, ORCHARD_FOUNDRY_CRATE_VERSION, ProviderOverride,
    document_catalog_refs, validate_foundry_document, verify_catalog_content_fingerprint,
};
...
pub struct FoundryProjectLoadContext {
    ...
    /// Current orchard-foundry crate version.
    pub current_foundry_version: Option<String>,
    /// Current orchard-family-compile crate version, when known to the caller.
...
impl FoundryProjectLoadContext {
    ...
    pub fn current_runtime() -> Self {
        Self {
            current_foundry_version: Some(ORCHARD_FOUNDRY_CRATE_VERSION.to_owned()),
            ..Self::default()
        }
    }
}
...
pub enum FoundryBuildStaleReason {
    ...
    /// The orchard-foundry crate version changed.
    ...
    /// The orchard-family-compile crate version changed.
...
pub enum FoundryProjectError {
    ...
    InvalidDocument {
        ...
        /// Full validation report from orchard-foundry.
...
fn apply_semantic_command(
    ...
    match command {
        ...
        FoundryCommand::ClearVariationFocus | FoundryCommand::ClearFocusPartGroup => {
            document.variation_state.intent.scope = orchard_foundry::VariationScope::WholeAsset;
            document.variation_state.intent = document.variation_state.intent.clone().normalized();
        }
        FoundryCommand::SetFocusPartGroup { group_id } => {
            document.variation_state.intent.scope =
                orchard_foundry::VariationScope::SemanticPartGroup {
