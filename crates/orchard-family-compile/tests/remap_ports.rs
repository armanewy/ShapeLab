...
fn family(rules: Vec<AttachmentRule>) -> AssetFamilySchema {
    ...
    AssetFamilySchema {
        ...
        part_roles: role_ids
            .into_iter()
            .map(|id| orchard_family::PartRole {
                id: id.clone(),
                display_name: id,
                required: true,
...
fn attachment_pairs(
    report: &orchard_family_compile::remap::ports::FragmentAttachmentBindingReport,
...
fn issue_codes(report: &orchard_family::FamilyValidationReport) -> BTreeSet<&str> {
    report
        .issues
        .iter()
