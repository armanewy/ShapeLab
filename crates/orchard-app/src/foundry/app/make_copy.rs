...
pub(super) struct MakeCanvasBannerContext<'a> {
    ...
    pub(super) candidate_ui_enabled: bool,
...
pub(super) fn empty_candidate_tray_copy(
    ...
    if !view_state.candidate_ui_enabled {
        return (
            "Adjustments ready",
            "Use available controls, Add to Pack, or Export.",
        );
    }
...
pub(super) fn try_ideas_action_label(profile_kind: MakeProfileKind) -> &'static str {
    match profile_kind {
...
pub(super) fn use_candidate_action_label(profile_kind: MakeProfileKind) -> &'static str {
...
pub(super) fn sphere_knob_like_form_preset_command() -> FoundryAppCommand {
    FoundryAppCommand::RunFoundryCommandProgram {
        ...
        commands: orchard_foundry_catalog::sphere_primitive::knob_like_form_preset_values()
            .into_iter()
...
pub(super) fn make_canvas_mode_summary(view_state: &MakeCanvasViewState) -> &'static str {
    match view_state.mode {
        ...
        MakeCanvasMode::FocusedPart if !view_state.candidate_ui_enabled => {
            "Adjust controls, Add to Pack, or Export."
        }
        ...
        MakeCanvasMode::Ready if !view_state.candidate_ui_enabled => {
            "Adjust controls, Add to Pack, or Export."
        }
