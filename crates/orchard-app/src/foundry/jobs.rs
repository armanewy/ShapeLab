...
use orchard_mesh::TriangleMesh;
use orchard_render::foundry::{
...
fn preview_control_value(
    value: &ControlValue,
) -> Option<orchard_render::foundry::FoundryPreviewControlValue> {
    match value {
        ControlValue::Scalar(value) => value.is_finite().then_some(
            orchard_render::foundry::FoundryPreviewControlValue::Scalar(*value),
        ),
        ControlValue::Integer(value) => {
            Some(orchard_render::foundry::FoundryPreviewControlValue::Integer(*value))
        }
        ControlValue::Toggle(value) => Some(
            orchard_render::foundry::FoundryPreviewControlValue::Toggle(*value),
        ),
        ControlValue::Choice(value) => Some(
            orchard_render::foundry::FoundryPreviewControlValue::Choice(value.clone()),
        ),
        ControlValue::Provider(value) => {
            Some(orchard_render::foundry::FoundryPreviewControlValue::Provider(value.clone()))
        }
    }
}
...
pub(crate) fn candidate_cards_from_output(
    ...
    selected: Option<&orchard_foundry::FoundryCandidateId>,
...
fn apply_candidate_preview(
    ...
    preview: &orchard_render::foundry::FoundryRenderedPreview,
) {
...
fn what_changed_summary(
    metadata: &CandidateVariationMetadata,
    explanations: &[orchard_search_internal::foundry::FoundryCandidateControlChange],
) -> String {
...
mod tests {
    ...
    fn test_candidate_card(id: &str, rgba8: Vec<u8>) -> FoundryCandidateCard {
        FoundryCandidateCard {
            id: orchard_foundry::FoundryCandidateId(id.to_owned()),
            slot: 0,
