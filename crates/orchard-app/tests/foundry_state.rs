...
use orchard_project::foundry::FoundryProjectFile;
use orchard_render::{OrbitCamera, foundry::FoundryPreviewCache};
...
fn run_candidate_cycle(
    ...
    fixture: &orchard_foundry_catalog::FoundryFixtureCatalog,
...
fn accepting_candidate_schedules_its_replayable_foundry_edit() {
    let mut state = FoundryAppState::new(minimal_foundry_document()).expect("valid state");
    let candidate_id = orchard_foundry::FoundryCandidateId("candidate-a".to_owned());
...
fn release_gate_all_builtin_profiles_render_real_option_thumbnails() {
    for fixture in orchard_foundry_catalog::headless_fixture_catalogs() {
        let state = compiled_fixture_state(&fixture);
...
fn undo_switches_to_parent_revision_and_rebuilds_off_thread() {
    ...
    assert_eq!(state.current_revision, Some(orchard_asset::RevisionId(0)));
    assert!(
        state
            .document
            .as_ref()
...
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("orchard-app lives two levels below the workspace")
        .to_path_buf()
}
...
fn provider_ref(stable_id: &str) -> CatalogContentRef {
    CatalogContentRef {
        ...
        schema_version: 1,
        fingerprint: orchard_family_compile::identity::CatalogContentFingerprint(
            orchard_family_compile::identity::ContentFingerprint([9; 32]),
        ),
    }
}
