...
use orchard_poly::{FaceMetadata, MeshBounds, PolyError, PolygonMesh, polygon_mesh_from_faces};
...
pub const ORCHARD_MODELING_CRATE_VERSION: &str = env!("CARGO_PKG_VERSION");
...
fn ensure_operations_supported(definition: &PartDefinition) -> Result<(), ModelingError> {
    for operation in &definition.geometry.operations {
        ...
        match operation {
            orchard_asset::ModelingOperationSpec::ReservedBoolean { operation, .. } => {
                return Err(ModelingError::UnsupportedOperation {
                    operation: *operation,
            ...
            orchard_asset::ModelingOperationSpec::ReservedDeformationProgram {
                operation, ..
            } => {
                return Err(ModelingError::UnsupportedOperation {
                    operation: *operation,
            ...
            orchard_asset::ModelingOperationSpec::TransformGeometry { .. }
            | orchard_asset::ModelingOperationSpec::SetBevelProfile { .. }
            | orchard_asset::ModelingOperationSpec::AddPanel { .. }
            | orchard_asset::ModelingOperationSpec::AddTrim { .. }
            | orchard_asset::ModelingOperationSpec::RecessedPanelCut { .. }
            | orchard_asset::ModelingOperationSpec::RectangularThroughCut { .. }
            | orchard_asset::ModelingOperationSpec::CircularThroughCut { .. }
            | orchard_asset::ModelingOperationSpec::BevelBoundaryLoop { .. }
            | orchard_asset::ModelingOperationSpec::MirrorInstances { .. }
            | orchard_asset::ModelingOperationSpec::LinearArray { .. }
            | orchard_asset::ModelingOperationSpec::RadialArray { .. } => {}
        }
    }
    Ok(())
}

fn operation_is_semantic_cut(operation: &orchard_asset::ModelingOperationSpec) -> bool {
    matches!(
        operation,
        orchard_asset::ModelingOperationSpec::RecessedPanelCut { .. }
            | orchard_asset::ModelingOperationSpec::RectangularThroughCut { .. }
