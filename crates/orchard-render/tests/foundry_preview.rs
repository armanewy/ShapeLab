...
use orchard_render::{OrbitCamera, RenderError, RenderSettings, RenderedImage};
...
fn preview_ids(previews: &[orchard_render::foundry::FoundryRenderedPreview]) -> Vec<&str> {
    previews
        .iter()
...
fn unique_foreground_rgbs(
    image: &orchard_render::RenderedImage,
    background: [u8; 4],
) -> BTreeSet<[u8; 3]> {
    image
        .rgba8
...
) -> orchard_render::foundry::FoundryPreviewBatchOutput {
...
fn assert_has_foreground(image: &orchard_render::RenderedImage, background: [u8; 4]) {
    assert!(
...
fn assert_camera_contains_bounds(camera: &orchard_render::OrbitCamera, bounds: Aabb) {
