...
use orchard_asset::{AssetRecipe, ModelingOperationSpec, PartInstanceId, validate_asset_recipe};
use orchard_compile::export::{
    verify_model_package, write_grouped_obj_export, write_model_package,
};
...
use orchard_core_legacy::{Aabb, ParamGroup, ShapeDocument, validate_document};
...
use orchard_mesh::{MeshSettings, TriangleMesh, mesh_field, write_obj_to_path};
use orchard_modeling_assets::{BenchmarkAsset, benchmark_assets};
use orchard_poly::{EdgeKey, PolygonMesh, TriangulatedPolygonMesh};
use orchard_presets::{PresetId, build_preset, list_presets};
use orchard_project::Project;
use orchard_render::foundry::FOUNDRY_DEFAULT_PREVIEW_CACHE_CAPACITY;
use orchard_render::{
...
use orchard_search_internal::{ExplorationMode, SearchRequest, TargetScope, generate_candidates};
use rayon::prelude::*;
use serde::Serialize;
...
enum Command {
    ...
    PrototypePack(prototype_pack_cli::PrototypePackArgs),
...
fn main() -> anyhow::Result<()> {
    ...
    match Cli::parse().command {
        ...
        Command::PrototypePack(args) => prototype_pack_cli::run_prototype_pack(args),
...
fn render_foundry_output_readability(
    output: &orchard_foundry::FoundryCompilationOutput,
    size: u32,
...
fn release_readiness_report(
    ...
    Ok(ReleaseReadinessReport {
        ...
        verification_commands: vec![
            ...
            "cargo run -p orchard-cli -- release-readiness --verify-product-ui-gate --out target/release-readiness-product-ui.json",
            "cargo test -p orchard-cli release_readiness",
...
fn unverified_release_product_ui_gate() -> ReleaseProductUiGate {
    ReleaseProductUiGate {
        ...
        verification_command: "orchard-cli release-readiness --verify-product-ui-gate",
...
fn verified_release_product_ui_gate() -> anyhow::Result<ReleaseProductUiGate> {
    let report = orchard_app::visual_foundry_product_ui_gate_report()
...
fn unverified_release_visual_product_gate() -> ReleaseVisualProductGate {
    ReleaseVisualProductGate {
        verification_status: "not-run",
        verification_command: "orchard-cli release-readiness --verify-visual-gate",
        native_state_verification_status: "requires-explicit-app-test",
        native_state_verification_command: "cargo test -p orchard-app release_gate_all_builtin_profiles_render_real_option_thumbnails -- --ignored",
        ...
        deterministic_contact_sheets: "orchard-cli foundry-visual-benchmark",
        evidence: None,
    }
}
...
fn orbit_camera_is_finite(camera: &orchard_render::OrbitCamera) -> bool {
...
fn print_author_validation_report(report: &orchard_foundry_catalog::FoundryAuthorValidationReport) {
...
fn render_asset_artifact_for_author_camera(
    artifact: &orchard_compile::AssetArtifact,
    camera: &orchard_foundry_catalog::FoundryAuthorPreviewCamera,
) -> anyhow::Result<RenderedImage> {
    let preview_mesh = render_mesh_from_triangles(&artifact.combined_preview);
...
fn select_visual_candidates(
    ...
    let mut candidate_by_id =
        BTreeMap::<String, (SemanticAssetCandidate, orchard_compile::AssetArtifact)>::new();
...
fn artifact_mesh_summary(artifact: &orchard_compile::AssetArtifact) -> MeshSummary {
    MeshSummary {
...
fn horizontal_mask_symmetry(mask: &[u64; orchard_render::VISUAL_DESCRIPTOR_MASK_WORDS]) -> f32 {
    let size = orchard_render::VISUAL_DESCRIPTOR_MASK_SIZE as usize;
...
fn mask_bit(mask: &[u64; orchard_render::VISUAL_DESCRIPTOR_MASK_WORDS], index: usize) -> bool {
    mask.get(index / 64)
        .is_some_and(|word| (word & (1_u64 << (index % 64))) != 0)
}

fn asset_symmetry_score(recipe: &AssetRecipe, artifact: &orchard_compile::AssetArtifact) -> f32 {
...
fn asset_topology_cost(recipe: &AssetRecipe, artifact: &orchard_compile::AssetArtifact) -> f32 {
...
fn near_coincident_ratio(issues: &[orchard_compile::validation::ValidationIssue]) -> f32 {
    let count = issues
        .iter()
        .filter(|issue| {
...
fn print_timeline(timeline: &orchard_compile::ConstructionTimelineReport) {
...
fn print_validation(
    artifact: &orchard_compile::AssetArtifact,
    model_report: &orchard_compile::validation::ModelValidationReport,
) {
    println!("Validation:");
    println!(
        "  compile validation: {}",
...
fn summarize_changes(operations: &[orchard_core_legacy::SetScalarEdit]) -> Vec<ChangedParameter> {
    operations
        .iter()
