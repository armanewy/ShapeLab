...
pub const ORCHARD_FOUNDRY_CATALOG_CRATE_VERSION: &str = env!("CARGO_PKG_VERSION");
...
fn build_fixture_catalog(spec: FixtureCatalogSpec) -> FoundryFixtureCatalog {
    ...
    let mut document = FoundryAssetDocument {
        schema_version: orchard_foundry::FOUNDRY_ASSET_DOCUMENT_SCHEMA_VERSION,
        ...
        variation_state: orchard_foundry::FoundryVariationState::default(),
    ...
    document.catalog_lock = Some(FoundryCatalogLock {
        ...
        compiler_version: orchard_foundry::ORCHARD_FOUNDRY_CRATE_VERSION.to_owned(),
        catalog_version: 1,
    });
...
fn default_export_requirement() -> orchard_family::ExportRequirement {
    orchard_family::ExportRequirement {
...
fn fragment(
    ...
    for (index, (path, minimum, maximum, step)) in scalar_paths.iter().enumerate() {
        ...
        recipe.parameters.insert(
            orchard_asset::ParameterId(parameter_id),
            scalar_parameter(
                parameter_id,
...
mod tests {
    use super::*;
    use orchard_family_compile::identity::{CatalogContentFingerprint, ContentFingerprint};
    use orchard_foundry::CatalogContentRef;
    ...
    fn headless_fixtures_compile_through_foundry() {
        for fixture in headless_fixture_catalogs() {
            ...
            let model_config =
                orchard_compile::validation::validation_config_from_recipe_with_limits(
                    &output.recipe,
                    &output.artifact,
                    orchard_compile::validation::ValidationLimits::default(),
                );
            let model_report =
                orchard_compile::validation::validate_model(&output.artifact, &model_config);
            assert!(
            ...
            assert!(
                output
                    ...
                    .any(|row| row.profile == "canonical-model-package"
                        && row.status
                            == orchard_family_compile::conformance::ConformanceStatus::Passed),
    ...
    fn visually_disconnected_parts(
        parts: &[orchard_compile::CompiledPart],
    ...
    fn effective_family_request_contains_defaulted_slots() {
        ...
        let output = orchard_foundry::compile_foundry_document(&document, &fixture)
            .expect("defaulted required control should compile");
        assert_eq!(
            output.family_request.parameters.get("proportions"),
            Some(&orchard_family_compile::FamilyValue::Choice(
    ...
    fn local_override_divergence_is_reported() {
        use orchard_asset::{AssetEdit, AssetEditProgram};
        use orchard_family_compile::identity::GeometryInputFingerprint;
        use orchard_foundry::{
        ...
        let base = orchard_foundry::compile_foundry_document(&fixture.document, &fixture)
        ...
        assert_eq!(
            ...
            Some(&orchard_foundry::ControlDivergence::DivergedByOverride)
        );
