...
use orchard_family::{AssetFamilySchema, RoleProvision, StyleKit};
use orchard_family_compile::{FamilyImplementation, StyleImplementation};
use orchard_foundry::{
...
fn kit_package_from_fixture(label: &str, fixture: &FoundryFixtureCatalog) -> FoundryKitPackage {
    ...
    let customizer: orchard_foundry::CustomizerProfile =
    ...
    FoundryKitPackage {
        schema_version: orchard_foundry::FOUNDRY_KIT_PACKAGE_SCHEMA_VERSION,
        kit: FoundryKit {
            schema_version: orchard_foundry::FOUNDRY_KIT_SCHEMA_VERSION,
            kit_id: kit_id.clone(),
...
fn family_blueprint_from_schema(family: &AssetFamilySchema) -> FamilyBlueprint {
    ...
    let attachment_expectations = family
        .attachment_rules
        .iter()
        .map(|rule| orchard_foundry::AttachmentExpectation {
    ...
    FamilyBlueprint {
        ...
        export_part_naming_policy: orchard_foundry::ExportPartNamingPolicy {
...
fn control_profile_from_customizer(
    ...
    family: &AssetFamilySchema,
    style_pack_id: &str,
    customizer: &orchard_foundry::CustomizerProfile,
) -> ControlProfile {
    ControlProfile {
...
fn control_profile_control(control: &orchard_foundry::CustomizerControl) -> ControlProfileControl {
...
#[cfg(test)]
mod tests {
    use orchard_foundry::{FoundryKitQualityTier, validate_foundry_kit_package};
