...
fn control_change_matches_group(
    ...
    match group.group_id.as_str() {
        ...
        "edges" => contains_any(&text, &["edge", "softness", "bevel"]),
        _ => false,
    }
}
...
fn descriptor_input_from_output(
    ...
    input.recipe_valid = orchard_asset::validate_asset_recipe(&output.recipe).is_valid();
...
fn repeated_element_count(recipe: &orchard_asset::AssetRecipe) -> usize {
    recipe
        .definitions
        .values()
...
fn bevel_radii(recipe: &orchard_asset::AssetRecipe) -> Vec<f32> {
...
fn topology_cost(recipe: &orchard_asset::AssetRecipe) -> f32 {
...
#[cfg(test)]
mod tests {
    use orchard_foundry::VariationIntent;
