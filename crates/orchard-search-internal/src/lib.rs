...
fn build_proposal(
    ...
    let candidate_document = orchard_core_legacy::apply_edit(document, &edit)
...
fn mutable_parameters(
    ...
    let mut descriptors: Vec<ParamDescriptor> = orchard_core_legacy::enumerate_parameters(document)
        .into_iter()
        ...
        .filter(|descriptor| orchard_core_legacy::get_scalar(document, &descriptor.path).is_ok())
        .collect();
...
fn target_nodes(
    ...
    match request.target_scope {
        ...
        TargetScope::Subtree => {
            ...
            for descendant in orchard_core_legacy::descendants_of(document, selected)? {
...
fn build_edit(
    ...
    for param_index in selected_indices {
        ...
        let before = orchard_core_legacy::get_scalar(document, &descriptor.path)?;
...
fn normalized_parameter_vector(
    ...
    descriptors
        .iter()
        .map(|descriptor| {
            orchard_core_legacy::get_scalar(document, &descriptor.path)
...
mod tests {
    use super::*;
    use orchard_core_legacy::{ParamPath, ShapeNode, Transform3, validate_document};
    ...
    fn locked_values_never_change() {
        ...
        let locked_before = orchard_core_legacy::get_scalar(&document, &locked)
        ...
        for candidate in results {
            ...
            let locked_after = orchard_core_legacy::get_scalar(&candidate.document, &locked)
                .unwrap_or_else(|error| panic!("locked parameter missing: {error}"));
    ...
    fn search_source_avoids_category_specific_terms() {
        ...
        for term in banned_terms {
            assert!(
                !source_terms.contains(&term.as_str()),
