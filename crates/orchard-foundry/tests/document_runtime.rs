...
fn catalog_fingerprint_mismatch_is_reported() {
    ...
    assert!(matches!(
        error,
        orchard_foundry::FoundryCompilationError::Catalog(
    ...
    assert!(
        output
            ...
            .all(|content| content.source
                == orchard_foundry::FoundryCatalogContentSource::EmbeddedSnapshot)
    );
...
fn final_conformance_rejects_missing_required_export_metadata() {
    ...
    assert!(matches!(
        error,
        orchard_foundry::FoundryCompilationError::FinalConformanceRejected(report)
            if report
                .exports
                .iter()
...
fn document_catalog_lock_rejects_unused_exact_refs() {
    ...
    assert!(matches!(
        error,
        orchard_foundry::FoundryCompilationError::DocumentValidationFailed(report)
            if report
                .issues
                .iter()
...
fn incompatible_style_is_rejected_by_family_compile() {
    ...
    assert!(matches!(
        error,
        orchard_foundry::FoundryCompilationError::FamilyCompile(_)
    ));
}
...
fn pinned_override_rejects_changed_style() {
    ...
    assert!(matches!(
        error,
        orchard_foundry::FoundryCompilationError::LocalOverrideRejected {
            reason,
            ..
...
fn command_replay_applies_non_candidate_commands_before_compile() {
    ...
    assert_eq!(
        ...
        Some(&orchard_family_compile::FamilyValue::Scalar(0.2))
    );
    assert!(
        fixture
            .document
...
impl RuntimeFixture {
    fn new() -> Self {
        ...
        let (profile_ref, profile_json) = catalog_entry(
            ...
            orchard_foundry::CUSTOMIZER_PROFILE_SCHEMA_VERSION,
            &profile,
        );
...
fn family_schema() -> AssetFamilySchema {
    AssetFamilySchema {
        ...
        parameter_slots: vec![orchard_family::FamilyParameterSlot {
            id: "radius".to_owned(),
...
fn provider_fragment(id: &str, radius: f32) -> RecipeFragment {
    RecipeFragment {
        schema_version: orchard_family_compile::RECIPE_FRAGMENT_SCHEMA_VERSION,
        id: id.to_owned(),
...
fn radius_override(
    id: &str,
    base_geometry_fingerprint: orchard_family_compile::identity::GeometryInputFingerprint,
    ...
    LocalRecipeOverride {
        ...
        touched_targets: vec![orchard_foundry::TouchedSemanticTarget::Parameter(parameter)],
        survival_policy,
    }
}
...
fn catalog_entry<T: Serialize>(
    ...
    let fingerprint =
        orchard_foundry::catalog_content_fingerprint_from_json(stable_id, &canonical_json)
