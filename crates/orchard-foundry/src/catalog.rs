...
use orchard_family::{AssetFamilySchema, StyleKit};
...
use crate::{CustomizerProfile, FoundryAssetDocument, ORCHARD_FOUNDRY_CRATE_VERSION};
...
impl FoundryCatalogLock {
    ...
    pub fn from_document_refs(document: &FoundryAssetDocument) -> Self {
        Self {
            ...
            compiler_version: ORCHARD_FOUNDRY_CRATE_VERSION.to_owned(),
            catalog_version: 0,
        }
    }
}
...
pub fn resolve_foundry_catalog(
    ...
    let family = decode_locked_content::<AssetFamilySchema>(
        &family_content,
        "orchard_family::AssetFamilySchema",
    )?;
    let style_kit = decode_locked_content::<StyleKit>(&style_content, "orchard_family::StyleKit")?;
    let family_implementation = decode_locked_content::<FamilyImplementation>(
        &family_impl_content,
        "orchard_family_compile::FamilyImplementation",
    )?;
    let style_implementation = decode_locked_content::<StyleImplementation>(
        &style_impl_content,
        "orchard_family_compile::StyleImplementation",
    )?;
    let customizer_profile = decode_locked_content::<CustomizerProfile>(
        &profile_content,
        "orchard_foundry::CustomizerProfile",
    )?;
