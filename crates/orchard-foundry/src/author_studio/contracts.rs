...
pub enum AuthorQualityGateTask {
    /// `orchard-cli foundry-kit validate`.
    ValidateKit,
    /// `orchard-cli foundry-kit preview`.
    RenderPreview,
    /// `orchard-cli foundry-kit contact-sheet`.
    RenderContactSheet,
    /// `cargo test -p orchard-foundry-catalog --test box_primitive --jobs 1`.
    BoxPrimitiveGate,
    /// `orchard-cli foundry-kit review`.
    ProduceReviewManifest,
    /// `orchard-cli foundry-kit package`.
    PackageKit,
}
