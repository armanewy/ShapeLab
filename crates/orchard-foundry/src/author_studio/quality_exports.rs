    ...
    vec![
        if let Some(kit_arg) = current_package_arg {
            supported(
                ...
                format!("orchard-cli foundry-kit validate {kit_arg}"),
            )
        } else {
            unsupported(
                AuthorQualityGateTask::ValidateKit,
        ...
        if let (Some(kit_arg), true) = (full_package_arg, has_out_dir) {
            supported(
                ...
                format!("orchard-cli foundry-kit preview {kit_arg} --out-dir {out_dir}/preview"),
            )
        } else {
            unsupported(
                AuthorQualityGateTask::RenderPreview,
        ...
        if let (Some(kit_arg), true) = (full_package_arg, has_out_dir) {
            supported(
                ...
                format!(
                    "orchard-cli foundry-kit contact-sheet {kit_arg} --out-dir {out_dir}/contact-sheet"
                ),
            )
        } else {
            unsupported(
        ...
        if let (Some(_kit_arg), true, Some(slug)) = (
            ...
            supported(
                ...
                format!(
                    "cargo test -p orchard-foundry-catalog --test box_primitive --jobs 1 # {slug} -> {out_dir}/box-primitive-gate"
                ),
            )
        } else {
            unsupported(
        ...
        match (&artifacts.quality_report_ref, full_package_arg, has_out_dir) {
            (Some(report_ref), Some(kit_arg), true) => supported(
                ...
                format!(
                    "orchard-cli foundry-kit review {kit_arg} --quality-report {report_ref} --out {out_dir}/review-manifest.json"
                ),
            ),
            _ => unsupported(
                AuthorQualityGateTask::ProduceReviewManifest,
        ...
        if !has_out_dir {
            ...
            supported(
                AuthorQualityGateTask::PackageKit,
                format!("orchard-cli foundry-kit package {kit_arg} --out-dir {out_dir}/package"),
            )
