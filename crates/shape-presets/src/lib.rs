...
pub enum PresetError {
    ...
    #[error("preset {id} failed validation with {issue_count} issue(s)")]
    InvalidPreset {
        /// Preset identifier.
        id: String,
...
pub fn list_presets() -> Vec<PresetMetadata> {
    vec![
        metadata(
            "desk-lamp",
            "Desk Lamp",
            "Adjustable desk lamp with a base, linked stems, and a broad shade.",
        ),
        metadata(
            "toy-submarine",
            "Toy Submarine",
            "Rounded toy submarine with porthole cutouts, tower, and fins.",
        ),
        metadata(
            "alien-plant",
            "Alien Plant",
            "Stylized plant form with a bulb, branching capsules, and pods.",
        ),
    ]
}
...
mod tests {
    ...
    fn stable_preset_listing() {
        ...
        assert_eq!(ids, vec!["desk-lamp", "toy-submarine", "alien-plant"]);
    }
