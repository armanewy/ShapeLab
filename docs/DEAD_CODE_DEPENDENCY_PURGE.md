...
is now `orchard-asset::AssetRecipe`, Orchard IR, `AuthoringOpLog`,
`RelationshipContract`, `PatternContract`, ObjectPlan materialization, and
geometry/export reports.

## Removed CLI Surface

- `orchard-cli decompile`
- `orchard-cli verify-decompile`
...
- obsolete `fixtures/orchard-asset/*_asset_recipe_v*.json` migration samples from
...
- `orchard-core-legacy` remains for legacy/low-level types until the separate retirement
  pass finishes its dependency audit.
- `orchard-search-internal` remains as an internal crate because active workspace members
  still depend on it.
...
- Continue reducing the `orchard-core-legacy` surface through the dedicated retirement
  branch.
