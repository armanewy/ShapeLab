...
The default novice UI may show Complete Looks and Shape. Surface may become a visual direction mode only when a real surface capability sidecar exists and the app can render and compare textured/material candidate previews. Rig, Motion, Gameplay, and Custom are reserved future/internal channels and are not current product features.
...
Shape candidates cannot pass by changing only materials. Surface candidates cannot claim shape changes. Complete Looks may vary shape and surface together when both channels are visually supported. Sci-Fi Crate has a Surface Lab v1 static-prop package path, but Surface mode remains unavailable with plain copy: "Surface package export exists for this kit. Visual surface variation needs textured previews and material candidate support."
