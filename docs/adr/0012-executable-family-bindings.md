...
Export roots must be disjoint. A fragment cannot list nested occurrence roots, overlapping occurrence roots, internal roots inside exported occurrence subtrees, or exported occurrence roots inside internal subtrees. Cardinality uses effective enabled state through ancestor chains. Fragment definitions, instances, parameters, operations, regions, boundary loops, sockets, relationships, locks, and variation metadata are remapped through typed semantic ID maps before merge.

Cross-fragment attachment bindings are explicit parent/child contracts. `AttachmentRule::from_role` is the child side and `AttachmentRule::to_role` is the parent side; `FragmentAttachmentBinding` stores `parent_role`/`parent_port` and `child_role`/`child_port` directly. The binding carries a rigid offset made of finite translation plus a canonical normalized quaternion. Scale is not part of socket alignment.

The compiler derives the instantiated `AssetId` from a domain-separated BLAKE3 geometry-input fingerprint. Geometry identity includes only inputs currently consumed by executable recipe generation: the base recipe geometry inputs, selected providers, selected fragment content, active export selectors, required-binding parameter values, executable parameter bindings, and cross-fragment attachment bindings. Request seed, advisory style policies, validation-only family constraints, recipe-level constraints and relationships, export requirements, selected-fragment export tags, and advisory/runtime-only parameter values do not change `AssetId` until an executable compiler path consumes them. Instantiation reports expose separate foundry-intent, geometry-input, conformance-contract, build, recipe, and artifact fingerprints as 64-character lowercase hex strings.
