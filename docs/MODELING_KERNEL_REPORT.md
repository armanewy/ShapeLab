...
`industrial-crate` now contains a rounded primary body, raised panels, corner reinforcement trim, mirrored swept handles, linear fastener arrays, feet, skid rails, and an optional top vent group. The compiled asset validates with complete face provenance and semantic region coverage, no SDF/remeshing, and no accidental intersections.
