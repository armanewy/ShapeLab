...
1. Ordered local operation execution and multi-cut composition
   ...
   - Present: Plate and RoundedBox cut controls derive feasible ranges from host face bounds, host thickness, strictly positive rim clearance, and topology locks.
   ...
   - Present: multiple same-face Plate or RoundedBox cuts are composed when their frames are separated and their rectangular projections do not split another cut window.
...
2. Boundary-loop bevels
   ...
   - Present: `BevelBoundaryLoop` consumes supported Plate and RoundedBox cut entry, exit, and recess-floor loops, emits two replacement loops, and generates bevel-band faces with deterministic segment/profile controls.
...
3. Rounded-box face cuts
   - Present: the controlled cut path supports flat primary patches of rounded-box faces, including recessed panels, rectangular through-cuts, circular through-cuts, and boundary-loop bevels on generated loops.
   - Present: recipe validation rejects rounded-box cut sets that target multiple primary faces in one definition, matching the current generator contract.
   - Next: support multi-face rounded-box cut batches once the local face subdivision composer can merge independent primary patches safely.
